...
5. 支持进入单词记忆模式，/*总词库分为n(n>=1)个大组，每个大组内有一定数量的单词，每次记忆训练时选择一个大组进行记忆。单次训练时是在这个大组中选去一定数量的单词，每次产生一个单词，用户输入是否认识，认识或不认识均进行记录，当一个单词连续几次被标为认识或不认识时调整难度标级，不同难度的单词需要出现在单次训练时的频率不同。*/
有多个记忆模式。
